 ...
 */
class FileChunker {
  constructor(file, chunkSize = 2 * 1024 * 1024) { // 默认2MB
    this.file = file;
...
app.post('/api/upload/chunk', upload.single('chunk'), async (req, res) => {
  try {
    const { fileHash, fileName, chunkIndex, totalChunks } = req.body;
...
大文件上传是一个复杂的技术挑战，需要综合考虑性能、稳定性、安全性等多个方面。核心技术包括文件切片、断点续传、并发控制、错误处理等。在实际项目中，还需要根据具体的业务场景和技术栈进行优化和调整。
