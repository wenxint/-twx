...
function arrayRandomAccess(arr, index) {
    return arr[index]; // 直接通过索引访问，时间与数组长度无关
}

// 调用示例
const fruits = ['apple', 'banana', 'cherry', 'date'];
console.log(arrayRandomAccess(fruits, 2)); // 输出: 'cherry'
```
...
console.log(binarySearch(sortedArr, 23)); // 输出: 5
...
arrayTraversal(numbers); // 依次输出: 10, 20, 30, 40, 50
```
